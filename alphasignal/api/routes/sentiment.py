...
async def get_sentiment(
    ...
    if not (1 <= len(ticker) <= 5):
        raise HTTPException(
            status_code=422,
            detail="Ticker must be 1-5 characters"
        )
    ...
    if ticker not in valid_tickers:
        raise HTTPException(
            status_code=404,
            detail=f"Ticker {ticker} not found in knowledge base"
        )
...
async def get_sentiment_summary(
    ...
    try:
        ...
        logger.error(f"Error getting sentiment summary for {ticker}: {e}", exc_info=True)
        raise
