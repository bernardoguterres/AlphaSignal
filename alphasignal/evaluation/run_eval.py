...
def _fetch_price_data_yfinance(
    ...
    # ticker, so df["Close"] is itself a 1-column DataFrame, not a Series -
    # iterating it directly yields column NAMES ('AAPL'), not prices, which
    # is exactly how this previously failed with "could not convert string
    # to float: 'AAPL'". Select the ticker's column explicitly to get a
    # real Series of floats.
...
def _get_nearest_trading_day(
    prices: dict[date, float], target: date, max_offset: int = 5
) -> Optional[date]:
...
def _compute_5d_forward_return(
    ...
    """
    try:
        prices = _fetch_price_data_yfinance(
            ticker, event_date, event_date + timedelta(days=30)
        )
    except ImportError:
    ...
    if entry_date is None:
        logger.warning(
            "No entry trading day found for %s around %s", ticker, event_date
        )
        return None
    ...
    if exit_idx >= len(sorted_dates):
        logger.warning(
            "Insufficient price history for 5-day forward return (%s, %s)",
            ticker,
            event_date,
        )
        return None
...
def _fetch_predicted_sentiment(
    ...
    try:
        ...
        # API not running, wrong/missing key, or other error - graceful
        # fallback, but log the real reason instead of silently swallowing
        # it (a previous version of this function hid a 401 behind
        # "unavailable" with no diagnostic when ALPHASIGNAL_API_KEY auth was
        # enabled but this client never sent X-API-Key).
...
def _print_results_table(results: list[dict[str, Any]]) -> None:
    ...
    for r in results:
        event_short = (
            (r["event_description"][:35] + "...")
            if len(r["event_description"]) > 38
            else r["event_description"]
        )
        ret_str = (
            f"{r['actual_5d_return_pct']:+.2f}%"
            if r["actual_5d_return_pct"] is not None
            else "N/A"
        )
...
def _print_summary(results: list[dict[str, Any]]) -> None:
    ...
    positive_returns = [
        ...
        if r["expected_sentiment"] == "positive"
        and r["actual_5d_return_pct"] is not None
    ]
    negative_returns = [
        r["actual_5d_return_pct"]
        for r in with_returns
        if r["expected_sentiment"] == "negative"
        and r["actual_5d_return_pct"] is not None
    ]

    avg_pos = (
        sum(positive_returns) / len(positive_returns) if positive_returns else None
    )
    avg_neg = (
    ...
    if len(signal_returns) >= 2:
        ...
        variance = sum((x - mean_ret) ** 2 for x in signal_returns) / (
            len(signal_returns) - 1
        )
    ...
    unavailable_count = sum(
        1 for r in results if r["predicted_sentiment"] == "unavailable"
    )
...
def run_evaluation(
    ...
    Args:
        ...
        api_key: X-API-Key to send if the target API has ALPHASIGNAL_API_KEY
            auth enabled. Defaults to the ALPHASIGNAL_API_KEY env var.
    ...
    api_key = api_key or os.getenv("ALPHASIGNAL_API_KEY")
    logging.basicConfig(
    ...
    for i, entry in enumerate(golden_set, start=1):
        ...
        print(
            f"[{i:2d}/{len(golden_set)}] {ticker:5s} {event_date_str}  Fetching returns...",
            end="",
            flush=True,
        )
